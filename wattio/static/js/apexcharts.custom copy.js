document.addEventListener('DOMContentLoaded', function () {
    ...
    const apiURL = `/plant/${plantID}/`; // Use the Django URL for the plant

    try {
        const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;

        // Sending the selected month to the Django backend
        ...
        if (responseData.status === 'success' && responseData.monthData) {
            // Assuming responseData.monthData is an array of objects with serial_number and data_list
            const seriesData = responseData.monthData.map((item, index) => ({
                name: item.serial_number,
                data: item.data_list.map(dataItem => {
                    let date = new Date(dataItem.create_date); // Assuming create_date is in a valid format
                    return { x: date.getTime(), y: dataItem.data }; // Adjust the property 'data' as per your response structure
                }),
    ...
    async function fetchDataAndUpdateChart() {
        ...
        if (dateValue) {
            const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;
            try {
                // Sending the selected date to the Django backend
                const response = await fetch(apiURL, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    ...
                    body: JSON.stringify({
                        'selected_date': dateValue  // Send the selected date
                    })
                });
